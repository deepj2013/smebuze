...
export default function EditWarehousePage() {
  ...
  useEffect(() => {
    apiGet<Record<string, unknown>>(`inventory/warehouses/${id}`).then((res) => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    setLoading(true);
    const { error: err } = await apiPatch(`inventory/warehouses/${id}`, {
      name,
      code: code || undefined,
      address: line1 ? { line1 } : {},
    });
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-slate-900 mb-4">Edit warehouse</h1>
