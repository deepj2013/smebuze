...
export default function DashboardPage() {
  ...
  const inviteJoinUrl = onboarding?.tenantSlug ? `${baseUrl}/join?slug=${encodeURIComponent(onboarding.tenantSlug)}` : '';
    ...
    <div>
      ...
      {inviteJoinUrl && (
        <div className="mb-6 rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="font-semibold text-slate-900 mb-2">Invite your team</h2>
          <p className="text-sm text-slate-600 mb-2">Share this link so others can join your workspace:</p>
          <div className="flex gap-2 items-center">
            <code className="flex-1 rounded-lg bg-slate-100 px-3 py-2 text-xs text-slate-800 break-all">{inviteJoinUrl}</code>
            <button
              type="button"
              onClick={() => navigator.clipboard?.writeText(inviteJoinUrl)}
