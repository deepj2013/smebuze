    ...
    <div>
      ...
      <form onSubmit={submit} className="max-w-lg space-y-4 rounded-xl border border-slate-200 bg-white p-6">
        <div>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="Customer or company name" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${fieldErrors.email ? 'border-red-500' : 'border-slate-300'}`} placeholder="email@example.com" />
          {fieldErrors.email && <p className="mt-0.5 text-xs text-red-600">{fieldErrors.email}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Phone</label>
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${fieldErrors.phone ? 'border-red-500' : 'border-slate-300'}`} placeholder="10–15 digits" maxLength={15} />
          {fieldErrors.phone && <p className="mt-0.5 text-xs text-red-600">{fieldErrors.phone}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">GSTIN</label>
          <input type="text" value={gstin} onChange={(e) => setGstin(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm uppercase ${fieldErrors.gstin ? 'border-red-500' : 'border-slate-300'}`} placeholder="15-character GSTIN" maxLength={15} />
        ...
        <div>
          ...
          <input type="text" value={pincode} onChange={(e) => setPincode(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${fieldErrors.pincode ? 'border-red-500' : 'border-slate-300'}`} placeholder="6 digits" maxLength={6} inputMode="numeric" />
        ...
        <div className="border-t border-slate-200 pt-4">
          ...
          {contacts.map((c, i) => (
            <div key={i} className="mb-4 p-4 rounded-lg border border-slate-200 bg-slate-50/50 space-y-3">
              ...
              {fieldErrors[`contact_${i}`] && (
                <p className="text-xs text-red-600">{fieldErrors[`contact_${i}`]}</p>
              )}
            </div>
          ))}
